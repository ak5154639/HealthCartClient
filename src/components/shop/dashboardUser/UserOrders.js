    ...
    <Fragment>
      <thead>
        <tr>
          ...
          <th className="px-4 py-2 border">Action/Refund Id</th>
  ...
  return (
    <Fragment>
      <tr className="border-b">
        ...
        <span
            onClick={(e) => {dispatch({ type: "loading", payload: true });cancelOrderReq(order._id,dispatch)}}
            className="cursor-pointer hover:bg-gray-200 rounded-lg p-2 mx-1"
          >Cancel</span>
        </td>)}
        {(order.status === "Cancelled")&&(<td className="hover:bg-gray-200 p-2 text-center">
          <span className="cursor-pointer hover:bg-gray-200 rounded-lg p-2 mx-1">{order.refundId}</span>
        </td>)}
      </tr>
    </Fragment>
  );
